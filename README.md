...
**Momentum is a modular, backend-agnostic application framework for the modern JVM.**
...
Momentum separates **what you want to do** from **how it gets done.** `momentum-gfx`
defines the GPU abstraction; `momentum-opengl` is one implementation. Swap it for a
...
Momentum is organized as a strict layered graph. Each module depends only on the
...
Every public API in Momentum follows a small set of rules:
...
Momentum targets **desktop and server JVM.** It does not attempt to run on
...
```bash
git clone https://github.com/licphel/momentum.git
cd momentum
...
```groovy
repositories {
    maven { url = 'file:///path/to/momentum/build/repo' }
}
